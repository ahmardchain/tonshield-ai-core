...
export interface SlippageResolution {
  slippageBps: number;
  slippagePercent: number;
  source: 'inline' | 'token' | 'global' | 'emergency' | 'system_default';
  isEmergencyDoubled: boolean;
  sandwichWarning: boolean;
}
...
export class SlippageService {
  ...
  public resolveSlippage(
    userId: number,
    tokenAddress?: string,
    inlineOverrideBps?: number,
  ): SlippageResolution {
    /**
    ...
    if (tokenAddress !== undefined && tokenAddress !== '') {
      const tokenSetting = getSlippageSetting(this.db, userId, 'token', tokenAddress);
    ...
    const globalSetting = getSlippageSetting(this.db, userId, 'global');
  ...
  public resolveEmergencySlippage(userId: number, tokenAddress?: string): SlippageResolution {
    /**
    ...
    const emergencySetting = getSlippageSetting(this.db, userId, 'emergency');
    ...
    const clamped = Math.min(
      Math.max(doubled, EMERGENCY_FLOOR_BPS),
      EMERGENCY_CEILING_BPS,
    );
  ...
  public setGlobalSlippage(userId: number, slippageBps: number): void {
    /**
     * Sets the user's global slippage default.
     * Applies to all trades unless overridden by token-specific or inline setting.
     */
    upsertSlippageSetting(this.db, userId, slippageBps, 'global');
  }

  public setTokenSlippage(userId: number, tokenAddress: string, slippageBps: number): void {
    /**
     * Sets token-specific slippage for a single token address.
     * Overrides global setting for all trades on this token.
     */
    upsertSlippageSetting(this.db, userId, slippageBps, 'token', tokenAddress);
  }

  public setEmergencySlippage(userId: number, slippageBps: number): void {
    /**
     ...
     */
    upsertSlippageSetting(this.db, userId, slippageBps, 'emergency');
  }

  public resetSlippage(
    userId: number,
     ...
     */
    deleteSlippageSetting(this.db, userId, settingType, tokenAddress);
  }

  public getUserSlippageSummary(userId: number): string {
    /**
     * Returns a formatted summary of all current slippage settings for a user.
     */
    const settings = getAllSlippageSettings(this.db, userId);
    ...
    lines.push(
      '',
      'Commands:',
      '/slippage <percent>            — set global default',
      '/slippage <token> <percent>    — set token-specific',
      '/emergency_slippage <percent>  — set emergency exit tolerance',
      '/slippage reset                — reset to system default',
    );

    return lines.join('\n');
  }

  public static validateSlippageInput(input: string): SlippageValidationResult {
    /**
    ...
    if (parsed < MIN_SLIPPAGE_BPS / 100) {
      return {
        valid: false,
        slippageBps: 0,
        errorMessage: `Slippage too low. Minimum is ${
          MIN_SLIPPAGE_BPS / 100
        }% (${MIN_SLIPPAGE_BPS} BPS).`,
        sandwichWarning: false,
      };
    }
    ...
    if (parsed > MAX_SLIPPAGE_BPS / 100) {
      return {
        valid: false,
        slippageBps: 0,
        errorMessage: `Slippage too high. Maximum allowed is ${
...
export function parseInlineSlippage(args: string[]): number | undefined {
  /**
...
export function formatSlippageResolution(resolution: SlippageResolution): string {
  /**
   ...
   */
  const sourceLabel: Record<SlippageResolution['source'], string> = {
