...
import { scorePool } from '../risk/riskScore';
import type { VelocityGuard } from '../risk/velocityGuard';
import type { BubbleMapAnalyzer } from '../safety/bubbleMap';
import type { HolderAnalyzer } from '../safety/holderAnalyzer';
import { formatSmallPrice, formatTaxBadge } from '../safety/priceService';
import { formatPercent, riskEmoji } from '../safety/safetyCache';
import type { TokenSafetyReport, TokenSafetyScanner } from '../safety/tokenScanner';
import { formatScanCard } from '../safety/tokenScanner';
import {
  formatSlippageResolution,
  parseInlineSlippage,
  SlippageService,
} from '../ston/slippageService';
import type { StonClient } from '../ston/stonClient';
import { buildTonscanLink } from '../ston/swapBuilder';
import type { StonFiExecutor } from '../ston/swapBuilder';
import { agentWalletCrypto, type AgentWalletService } from '../wallet/agentWallet';
import type { WithdrawalEngine } from '../wallet/withdrawalEngine';
import { getAgentWallet, hasAgentWallet } from '../wallet/walletStore';
...
export interface BotCommandServices {
  ...
  executor: StonFiExecutor;
  tonClient: TonClient;
  withdrawalEngine: WithdrawalEngine;
  tokenScanner: TokenSafetyScanner;
  holderAnalyzer: HolderAnalyzer;
  bubbleMapAnalyzer: BubbleMapAnalyzer;
  slippageService: SlippageService;
}

interface PendingEmergencySell {
  userId: number;
  poolAddress: string;
  tokenAddress: string;
  amountTon: number;
...
interface PendingBuy {
  ...
  tokenAddress: string;
  amountTon: number;
  inlineSlippageBps?: number;
  createdAt: number;
}

interface BuyQuoteView {
  askAmount?: string;
  minAskAmount?: string;
...
interface PendingKeyExport {
  chatId: number;
  timeout: NodeJS.Timeout;
}

const pendingEmergencySells = new Map<string, PendingEmergencySell>();
const pendingSellNow = new Map<string, PendingEmergencySell>();
const pendingBuys = new Map<string, PendingBuy>();
const pendingKeyExports = new Map<number, PendingKeyExport>();
...
async function ensureTelegramUser(ctx: Context, db: DatabaseConnection): Promise<number | null> {
  const from = ctx.from;
  ...
  upsertUser(db, from.id, from.username ?? null);
  return from.id;
}

function formatNetwork(config: Config): string {
  return config.network === 'mainnet'
    ? `mainnet (execution ${config.enableMainnetExecution ? 'enabled' : 'disabled'})`
    : 'testnet';
}

function firstTokenAddress(tokenAddresses: string[]): string {
  return tokenAddresses[0] ?? 'unknown';
}

function parsePercent(value: string | undefined, label: string, max: number): number {
  const parsed = Number(value);
...
function pnlIndicator(value: number): string {
  if (value > 0) {
    return '🟢';
  }
...
function formatExecutionResult(
  ...
  if (status === 'success' && txHash !== undefined) {
    return [
      ...
      'Status: success',
      `🔗 Verify on TONscan: ${buildTonscanLink(txHash, config.network)}`,
    ].join('\n');
  }
...
function buildTonscanAddressLink(address: string, network: Config['network']): string {
  const base =
    network === 'testnet' ? 'https://testnet.tonscan.org/address' : 'https://tonscan.org/address';
  return `${base}/${address}`;
}

function assertNonEmpty(value: string | undefined, usage: string): string {
...
function parseTonAddress(address: string, label: string): void {
  try {
...
async function deriveAgentPublicKey(
  encryptedMnemonic: string,
  config: Config,
): Promise<string> {
  const mnemonic = agentWalletCrypto.decryptMnemonic(
    encryptedMnemonic,
    config.agentWalletEncryptionKey,
  );
  const keyPair = await mnemonicToPrivateKey(mnemonic.split(' '));
  return Buffer.from(keyPair.publicKey).toString('hex');
}

function formatMnemonicForExport(mnemonic: string): string {
...
export function formatSafetyReport(report: TokenSafetyReport, poolAddress = ''): string {
  if (poolAddress !== '__legacy_text__') {
    return formatScanCard(report, poolAddress);
  }

  const honeypotIcon =
    report.honeypot.result === 'PASS'
      ? '✅'
  ...
  const concentrationIcon =
    report.concentration.devWalletPercent > 20
      ? '🔴'
  ...
  const lines = [
    ...
    '',
    `Honeypot Risk:        ${honeypotIcon} ${report.honeypot.result} — Round-trip loss: ${formatPercent(report.honeypot.roundTripLossPercent)}`,
    ...
    `Supply Concentration: ${concentrationIcon} ${report.concentration.devWalletPercent > 20 ? 'FAIL' : 'PASS'} — Dev holds ${formatPercent(report.concentration.devWalletPercent)}`,
    '',
    `Overall Risk: ${riskEmoji(report.overallRisk)} ${report.overallRisk}`,
  ...
  if (report.overallRisk === 'CRITICAL') {
    lines.push(
      ...
      'This token has been flagged as extremely dangerous.',
      'Buying is strongly discouraged.',
    );
  }
...
function formatCondensedSafetySummary(report: TokenSafetyReport): string {
  const market = report.marketData;
  const taxLine =
    market !== null
      ? formatTaxBadge(market.buyTaxPercent, market.sellTaxPercent).replace(/^[^\s]+ /, '')
      : 'TAX   Buy: N/A | Sell: N/A';
  const priceLine = `💲 Price: ${
    market !== null ? `${formatSmallPrice(market.priceTon)} TON` : 'N/A'
  }`;
...
function formatSlippageWarning(sandwichWarning: boolean): string {
  return sandwichWarning
    ? '\n⚠️ High slippage — sandwich attack risk. Consider using a lower value.'
    : '';
}

function slippageUsage(): string {
  return [
    'Usage examples:',
    '/slippage',
    '/slippage 5',
...
async function calculateWithdrawableTon(
  services: BotCommandServices,
  agentAddress: string,
): Promise<number> {
  const balance = await services.tonClient.getBalance(Address.parse(agentAddress));
  const balanceTon = Number(balance) / 1e9;
  return Math.max(0, balanceTon - WITHDRAWAL_GAS_RESERVE_TON);
}

function quoteAskAmount(quote: unknown): string {
  const view = quote as BuyQuoteView;
  return view.askAmount ?? view.estimatedOutput ?? '0';
}

function quoteMinAskAmount(quote: unknown): string {
  const view = quote as BuyQuoteView;
  return view.minAskAmount ?? view.minimumReceived ?? '0';
}

function quoteEntryPrice(amountTon: number, quote: unknown): number {
  const view = quote as BuyQuoteView;
...
export function registerCommands(bot: Telegraf<Context>, services: BotCommandServices): void {
  bot.start(async (ctx) => {
  ...
  bot.command('create_agent_wallet', async (ctx) => {
    ...
    const existing = getAgentWallet(services.db, userId);
  ...
  bot.command('budget', async (ctx) => {
    ...
    try {
      ...
      await safeReply(
        ctx,
        `Budget rejected: ${error instanceof Error ? error.message : 'Invalid input.'}`,
      );
    }
  });
  ...
  bot.command('scan', async (ctx) => {
    ...
    try {
      ...
      const risk = velocity === null ? null : scorePool(velocity);
  ...
  bot.command('scan_token', async (ctx) => {
    ...
    try {
      ...
      const tokenAddress = assertNonEmpty(
        token,
        'Usage: /scan_token <token_address> <pool_address> [force]',
      );
      const poolAddress = assertNonEmpty(
        pool,
        'Usage: /scan_token <token_address> <pool_address> [force]',
      );
      ...
      const report = await services.tokenScanner.scanToken(
        tokenAddress,
        poolAddress,
        forceRefresh,
      );

      await safeReply(ctx, formatScanCard(report, poolAddress));
    } catch (error) {
      await safeReply(
        ctx,
        `Token scan failed: ${error instanceof Error ? error.message : 'Unknown token scan error.'}`,
      );
    }
  });
  ...
  bot.command('holders', async (ctx) => {
    ...
    try {
      ...
      await safeReply(
        ctx,
        `Holder analysis failed: ${error instanceof Error ? error.message : 'Unknown holder analysis error.'}`,
      );
    }
  });
  ...
  bot.command('bubbles', async (ctx) => {
    ...
    try {
      ...
      await safeReply(
        ctx,
        `🫧 Building wallet connection map for ${shortAddress(tokenAddress)} via ${shortAddress(poolAddress)}...`,
      );
      ...
      const quickReport = await services.bubbleMapAnalyzer.runQuickScan(
        tokenAddress,
        holders,
      ...
      const deepReport = await services.bubbleMapAnalyzer.runDeepScan(
        tokenAddress,
        holders,
        tokenDeployTimestamp,
        (message) => {
      ...
      await safeReply(
        ctx,
        `Bubble map analysis failed: ${error instanceof Error ? error.message : 'Unknown bubble map error.'}`,
      );
    }
  });
  ...
  bot.command('watch', async (ctx) => {
    ...
    addWatchedPool(services.db, userId, poolAddress);
    const count = listWatchedPools(services.db, userId).length;
  ...
  bot.command('unwatch', async (ctx) => {
    ...
    removeWatchedPool(services.db, userId, poolAddress);
  ...
  bot.command('watchlist', async (ctx) => {
    ...
    const watched = listWatchedPools(services.db, userId);
    const armed = new Set(listArmedPools(services.db, userId).map((pool) => pool.pool_address));
    ...
    const lines = watched.map((pool) => {
      ...
      const risk = velocity === null ? 'insufficient data' : scorePool(velocity).level;
  ...
  bot.command('settings', async (ctx) => {
    ...
    try {
      ...
      if (poolAddress === undefined || poolAddress.trim().length === 0) {
        throw new Error(
          'Usage: /settings <pool_address> <take_profit_percent> <stop_loss_percent>',
        );
      }
      ...
      let entryPriceTon: number | undefined;
      ...
      await safeReply(
        ctx,
        `Settings rejected: ${error instanceof Error ? error.message : 'Usage: /settings <pool_address> <take_profit_percent> <stop_loss_percent>'}`,
      );
    }
  });
  ...
  bot.command('slippage', async (ctx) => {
    ...
    try {
      ...
      if (args.length === 1) {
        const validation = SlippageService.validateSlippageInput(args[0] ?? '');
      ...
      parseTonAddress(tokenAddress, 'token_address');
      const validation = SlippageService.validateSlippageInput(percent);
      ...
      await safeReply(
        ctx,
        `Slippage update failed: ${error instanceof Error ? error.message : 'Invalid input.'}\n\n${slippageUsage()}`,
      );
    }
  });
  ...
  bot.command('emergency_slippage', async (ctx) => {
    ...
    try {
      ...
      await safeReply(
        ctx,
        `Emergency slippage update failed: ${
  ...
  bot.command('positions', async (ctx) => {
    ...
    const positions = getOpenPositions(services.db, userId);
    ...
    for (const position of positions) {
      ...
      const pnlPercent =
        ((currentPriceTon - position.entry_price_ton) / position.entry_price_ton) * 100;
      ...
      await ctx.reply(
        ...
        Markup.inlineKeyboard([
          [Markup.button.callback('Sell Now', `sell_now:${position.pool_address}`)],
        ]),
      );
    }
  });
  ...
  bot.command('history', async (ctx) => {
    ...
    const positions = getClosedPositions(services.db, userId, 10);
    ...
    for (const position of positions) {
      ...
      const tonscanLink =
        position.tx_hash_close === null
          ? 'unavailable'
          : buildTonscanLink(position.tx_hash_close, services.config.network);
  ...
  bot.command('preview_buy', async (ctx) => {
    ...
    try {
      ...
      const quote = await services.stonClient.getSwapQuote(
        tokenAddress,
        amountTon,
        services.config.defaultSlippageBps,
      );
      ...
      await safeReply(
        ctx,
        `Preview failed: ${error instanceof Error ? error.message : 'Usage: /preview_buy <pool_address> <token_address> <amount_ton>'}`,
      );
    }
  });
  ...
  bot.command('buy', async (ctx) => {
    ...
    if (!hasAgentWallet(services.db, userId)) {
    ...
    try {
      ...
      const amountTon = parsePositiveAmount(amount);
      const inlineSlippageBps = parseInlineSlippage(args);
      const slippageResolution = services.slippageService.resolveSlippage(
        userId,
        tokenAddress,
        inlineSlippageBps,
      );
      ...
      if (safetyReport.overallRisk === 'CRITICAL' || safetyReport.honeypot.result === 'FAIL') {
        await safeReply(
          ctx,
          [formatSafetyReport(safetyReport), '', '🚨 Buy blocked due to critical risk.'].join(
            '\n',
          ),
        );
        return;
      ...
      await ctx.reply(
        [
          ...
          `Token: ${tokenAddress}`,
          `Amount: ${amountTon} TON`,
          `Slippage: ${formatSlippageResolution(slippageResolution)}`,
        ...
        Markup.inlineKeyboard([
          [
            Markup.button.callback(
              '✅ Confirm Buy',
              `confirm_buy:${confirmationId}`,
            ),
            Markup.button.callback('❌ Cancel', `cancel_buy:${confirmationId}`),
          ],
        ]),
      );
    } catch (error) {
      await safeReply(
        ctx,
        `Buy rejected: ${error instanceof Error ? error.message : 'Usage: /buy <pool_address> <token_address> <amount_ton>'}`,
      );
    }
  });
  ...
  bot.command('arm', async (ctx) => {
    ...
    addWatchedPool(services.db, userId, poolAddress);
    addArmedPool(services.db, userId, poolAddress);
    await safeReply(
      ctx,
      [
  ...
  bot.command('disarm', async (ctx) => {
    ...
    removeArmedPool(services.db, userId, poolAddress);
  ...
  bot.command('agent_status', async (ctx) => {
    ...
    const wallet = getAgentWallet(services.db, userId);
    const budget = services.budgetPolicy.getPolicy(userId);
    const armed = listArmedPools(services.db, userId);
    const remaining =
      budget === undefined ? null : Math.max(0, budget.maxBudgetTon - budget.currentSpentTon);
  ...
  bot.command('withdraw_ton', async (ctx) => {
    ...
    try {
      ...
      const destinationAddress = assertNonEmpty(
        destination,
        'Usage: /withdraw_ton <destination_address> <amount_ton>',
      );
      ...
      await ctx.reply(
        ...
        Markup.inlineKeyboard([
          [
            Markup.button.callback(
              '✅ Confirm Withdrawal',
              `confirm_withdraw_ton:${destinationAddress}:${amountTon}`,
            ),
      ...
      await safeReply(
        ctx,
        `Withdrawal rejected: ${error instanceof Error ? error.message : 'Usage: /withdraw_ton <destination_address> <amount_ton>'}`,
      );
    }
  });
  ...
  bot.command('withdraw_all', async (ctx) => {
    ...
    try {
      ...
      await ctx.reply(
        ...
        Markup.inlineKeyboard([
          [
            Markup.button.callback('✅ Confirm', `confirm_withdraw_all:${destinationAddress}`),
            Markup.button.callback('❌ Cancel', 'cancel_withdrawal'),
          ],
        ]),
      );
      ...
      await safeReply(
        ctx,
        `Full withdrawal rejected: ${error instanceof Error ? error.message : 'Usage: /withdraw_all <destination_address>'}`,
      );
    }
  });
  ...
  bot.command('withdraw_token', async (ctx) => {
    ...
    try {
      ...
      const tokenAddress = assertNonEmpty(
        token,
        'Usage: /withdraw_token <token_address> <destination_address>',
      );
      const destinationAddress = assertNonEmpty(
        destination,
        'Usage: /withdraw_token <token_address> <destination_address>',
      ...
      await ctx.reply(
        ...
        Markup.inlineKeyboard([
          [
            Markup.button.callback(
              '✅ Confirm',
              `confirm_withdraw_token:${tokenAddress}:${destinationAddress}`,
            ),
            Markup.button.callback('❌ Cancel', 'cancel_withdrawal'),
          ],
        ]),
      ...
      await safeReply(
        ctx,
        `Token withdrawal rejected: ${error instanceof Error ? error.message : 'Usage: /withdraw_token <token_address> <destination_address>'}`,
      );
    }
  });
  ...
  bot.command('export_address', async (ctx) => {
    ...
    try {
      ...
      await safeReply(
        ctx,
        `Address export failed: ${error instanceof Error ? error.message : 'Unknown export error.'}`,
      );
    }
  });
    ...
    const timeout = setTimeout(() => {
      ...
      if (pending?.timeout === timeout) {
        ...
        void bot.telegram.sendMessage(
          chatId,
          '⏱️ Export request expired. Run /export_keys again if needed.',
        );
      }
    }, KEY_EXPORT_TTL_MS);
  ...
  bot.action(/^confirm_withdraw_ton:(.+):([^:]+)$/, async (ctx) => {
    ...
    try {
      ...
      const result = await services.withdrawalEngine.withdrawTon(
        userId,
        destinationAddress,
        amountTon,
      );
      ...
      if (result.status !== 'success' || result.txHash === undefined) {
        await safeReply(
          ctx,
          `Withdrawal failed: ${result.errorMessage ?? result.status}`,
        );
        return;
      }
      ...
      await safeReply(
        ...
        [
          ...
          `Sent: ${amountTon} TON`,
          `To: ${destinationAddress}`,
          `🔗 Verify: ${buildTonscanLink(result.txHash, services.config.network)}`,
        ].join('\n'),
      );
    } catch (error) {
      await safeReply(
        ctx,
        `Withdrawal failed: ${error instanceof Error ? error.message : 'Unknown withdrawal error.'}`,
      );
    }
  });
  ...
  bot.action(/^confirm_withdraw_all:(.+)$/, async (ctx) => {
    ...
    try {
      const wallet = getAgentWallet(services.db, userId);
      const amountTon =
        wallet === undefined ? 0 : await calculateWithdrawableTon(services, wallet.address);
  ...
  bot.action(/^confirm_withdraw_token:([^:]+):(.+)$/, async (ctx) => {
    ...
    try {
      ...
      const result = await services.withdrawalEngine.withdrawToken(
        userId,
        tokenAddress,
        destinationAddress,
      );
      ...
      if (result.status !== 'success' || result.txHash === undefined) {
        await safeReply(
          ctx,
          `Token withdrawal failed: ${result.errorMessage ?? result.status}`,
        );
        return;
      }
      ...
      await safeReply(
        ctx,
        `Token withdrawal failed: ${error instanceof Error ? error.message : 'Unknown token withdrawal error.'}`,
      );
    }
  });
  ...
  bot.command('emergency_sell', async (ctx) => {
    ...
    try {
      ...
      const pool = await services.stonClient.getPoolData(poolAddress);
      const tokenAddress = firstTokenAddress(pool.tokenAddresses);
      const emergencyResolution = services.slippageService.resolveEmergencySlippage(
        userId,
        tokenAddress,
      );
      const confirmationId = randomUUID();
      ...
      await ctx.reply(
        [
          ...
          '',
          `Pool: ${poolAddress}`,
          `Slippage: ${formatSlippageResolution(emergencyResolution)}`,
        ...
        Markup.inlineKeyboard([
          [
            Markup.button.callback(
              ...
              `emergency_sell_confirm:${confirmationId}`,
            ),
      ...
      await safeReply(
        ctx,
        `Emergency sell preparation failed: ${error instanceof Error ? error.message : 'Unknown error.'}`,
      );
    }
  });
  ...
  bot.action(/^emergency_sell_confirm:(.+)$/, async (ctx) => {
    ...
    const result = await services.executor.executeDefensiveSwap(
      pending.userId,
      pending.poolAddress,
  ...
  bot.action(/^confirm_buy:(.+)$/, async (ctx) => {
    ...
    try {
      ...
      await ctx.answerCbQuery(
        services.config.paperTrade ? 'Simulating buy...' : 'Executing buy...',
      );

      if (services.config.paperTrade) {
        const result = await services.executor.simulateBuySwap(
          pending.userId,
          pending.poolAddress,
          pending.tokenAddress,
          pending.amountTon,
          { inlineSlippageBps: pending.inlineSlippageBps },
        );
      ...
      const result = await services.executor.executeBuySwap(
        pending.userId,
        pending.poolAddress,
        pending.tokenAddress,
        pending.amountTon,
      ...
      openPosition(
        ...
        pending.poolAddress,
        pending.tokenAddress,
        result.entryPriceTon,
        pending.amountTon,
        result.txHash,
      );

      const alreadyWatched = listWatchedPools(services.db, pending.userId).some(
        (pool) => pool.pool_address === pending.poolAddress,
      );

      if (!alreadyWatched) {
        addWatchedPool(services.db, pending.userId, pending.poolAddress);
      }
  ...
  bot.action(/^sell_now:(.+)$/, async (ctx) => {
    ...
    const position = getOpenPositions(services.db, userId).find(
      (openPosition) => openPosition.pool_address === poolAddress,
    );
    ...
    await ctx.reply(
      `Confirm sell for ${poolAddress}.`,
      Markup.inlineKeyboard([
        [
          Markup.button.callback('Confirm Sell', `sell_now_confirm:${confirmationId}`),
          Markup.button.callback('Cancel', `sell_now_cancel:${confirmationId}`),
        ],
      ]),
    );
  });
  ...
  bot.on('text', async (ctx, next) => {
    ...
    try {
      logKeyExport(services.db, userId, 'seed_phrase');
      await services.agentWallet.decryptAndLoadWallet(userId);
      const mnemonic = agentWalletCrypto.decryptMnemonic(
        wallet.encryptedMnemonic,
        services.config.agentWalletEncryptionKey,
      );
      ...
      setTimeout(() => {
        void (async () => {
          try {
            ...
            await ctx.reply(
              '⚠️ Automatic deletion failed. Delete the seed phrase message manually now.',
            );
          }
        })();
      }, KEY_EXPORT_TTL_MS);
      ...
      await safeReply(
        ctx,
        `Key export failed: ${error instanceof Error ? error.message : 'Unknown key export error.'}`,
      );
    }
  });
}
